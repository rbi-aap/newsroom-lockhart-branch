...
def process_error_response(response):
    ...
    def syndicate_examples() -> Mapping[str, str]:
        examples = OrderedDict([
            ('json', f"{request.url_root}news/syndicate?format=json&q=trump&start_date=2020-04-01&timezone=Australia/Sydney"),
            ('atom', f"{request.url_root}news/syndicate?format=atom&start_date=now-30d&end_date=now&timezone=Austrlia/Sydney&include_fields=headline,byline,slugline,description_html,located,keywords,source,subject,place,wordcount,charcount,body_html,readtime,profile,service,genre,associations"),
            ('rss', f"{request.url_root}news/syndicate?format=rss&exclude_fields=version,versioncreated,firstcreated")
        ])
        return examples
